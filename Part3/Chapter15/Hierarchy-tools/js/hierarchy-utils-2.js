...
export function makeHierarchy(nested, name) {
    const root = {id: name, data: {population: 0, area: 0, km: 0}, children: []};
    const data = nested.get(name);
    return makeChildren(root, data ? nested.get(name) : nested)
}
...
function makeChildren(node, data) {
    ...
    entries.forEach(([key, value]) => {
            const child = {id: key, data: {population: 0, area: 0, km: 0}, children: []};
...
export function tabularToHierarchy(data, rootKey, reduceFunc, ...keyFunc) {
    // 1) Convert the tabular data to a nested object structure
    const nested = d3.rollup(data, reduceFunc, ...keyFunc);
