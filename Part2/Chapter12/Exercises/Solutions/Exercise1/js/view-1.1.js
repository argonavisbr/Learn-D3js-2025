...
export function draw() {
    ...
    // EXERCISE - Text labels: there are two possible solutions.

    // 1) Using labels on slices (remove if using legends)
/*
    utils.pieLabels()
 ...
 */
    ...
    // a) EXERCISE: Place the legend (a <g> container) in the top-right corner of the chart
    const legend = svg.append("g").attr("transform", `translate(${[dim.width-120,30]})`);

    // b) EXERCISE: Draw the legend (use utils.legend() from chart-utils.js) - use country names as the data
    utils.legend()
         .container(legend)
         .color(app.color)
