function draw(dim, grid, scale, easeType, index, transition, animated = false, shortName = false) {
    ...
    if(animated) {
        ...
        animate(easeType, scale, transition);
    }
}

function animate(easeType, scale, tr) {
    let repeat = true;
    if(!tr) {
        tr = d3.transition().duration(3000).delay(1000);
    } else {
        repeat = false; // don't repeat if transition is passed
    }
    d3.selectAll(`circle.line-dot.${easeType}`)
        .transition(tr)
        .attrTween("cx", () => t => scale.x(t))
        .attrTween("cy", () => t => scale.y(d3[easeType](t)))
        .on("end", () => {if (repeat) animate(easeType, scale)});
