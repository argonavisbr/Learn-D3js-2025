...
import {dim, chart, app, getTransition} from "./common-1.2.js";
...
function joinEnter(enter) {
    ...
    const enterGrp =
        enter.append("g")
            ...
            .style("opacity", initOpacity)            // enter the chart as a transparent element
...
function joinUpdate(update) {
    ...
    const updateGrp = update.transition(getTransition())    // changed to use reusable transition
                            .attr("transform", finalRank)
                            .style("opacity", 1)            // changed
...
function joinExit(exit) {
    ...
    const exitGrp = exit.transition(getTransition())        // changed to use reusable transition
                        .attr("transform", nextRank)
                        .style("opacity", 0)                // disappear while leaving the chart
