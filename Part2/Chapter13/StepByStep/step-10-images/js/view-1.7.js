...
export function draw() {
    ...
    // 6) Create clipping paths for the images
...
export function show(dataFrame) {
    ...
    d3.select(".step.label").text(Math.round(year)); // changed - rounding the year
}
...
function joinEnter(enter) {
    ...
    enterGrp.append("image")
        ...
        .attr("x", d => prevWidth(d) - app.scale.y.bandwidth() * (1 + 2/3))
...
function joinUpdate(update) {
    ...
    const updateGrp = update.transition(getTransition())   // changed - getting the transition
...
function joinExit(exit) {
    ...
    const exitGrp = exit.transition(getTransition())   // changed - getting the transition
...
function resizeBars(group, width, value) {
    ...
    group.select("image.bar").attr("x", d => width(d) - app.scale.y.bandwidth() * (1 + 2/3))
