...
const svg = d3.create("svg").attr("class", "main-svg")
              .attr("width", dim.width)
              .attr("height", dim.height)

const layer = svg.append("g");
const chart = layer.append("svg").attr("class", "chart");
...
function makeGrid(margin = 0, width = dim.width, height = dim.height) {
    ...
    layer.attr("transform", `translate(${[margin,margin]})`)
    chart.attr("width", width - margin * 2)
